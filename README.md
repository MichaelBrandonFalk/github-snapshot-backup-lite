...
GithubSnapshot V1.4 is a small macOS utility that keeps weekly point-in-time ZIP snapshots of every repository owned by the authenticated GitHub account.
...
- All repositories by default, scanned at backup time so newly created repositories are included
...
6. Leave Weekly enabled. The default is Wednesday at 10:00 AM; choose a day/time when the Mac is usually awake and online.
...
```text
dist/GithubSnapshot_V1.4.app
downloads/GithubSnapshot_V1.4_macOS.zip
```
...
```text
GithubSnapshot_V1.4.app/Contents/MacOS/GithubSnapshot_V1.4 --headless-backup
```
